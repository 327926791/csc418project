...
IGL_INLINE bool igl::readOBJ(
  const std::string obj_file_name,
	...
	std::vector < std::vector<std::vector<Scalar > > > & vTC,
	std::vector < std::vector<std::vector<Scalar > > > & vN,
	std::vector < std::vector<std::vector<Index > > > & vF,
	std::vector < std::vector<std::vector<Index > > > & vFTC,
	std::vector < std::vector<std::vector<Index > > > & vFN,
	...
	std::vector<std::string> texture_files)
{
  ...
  return igl::readOBJ(obj_file,vV,vTC,vN,vF,vFTC,vFN, soup_materials,texture_files);
}
  ...
  while (fgets(line, IGL_LINE_MAX, obj_file) != NULL)
  {
	  ...
	  if (sscanf(line, "%s", type) == 1)
	  {
		 // printf("type = %s\n", type);
		  ...
		  else if (strcmp("usemtl", type) == 0) {
			  char str[IGL_LINE_MAX], usemtl[IGL_LINE_MAX];
			  sscanf(line, "%s %s", str, usemtl);
		  ...
		  else if (strcmp("mtllib", type) == 0) {
			  printf("read mtllib");
			  char str[IGL_LINE_MAX], mtllib[IGL_LINE_MAX];
			  sscanf(line, "%s %s", str, mtllib);
			  //read_mtllib(file_name, soup_materials, texturefiles);
		  }
